...
public class BasketController(StoreContext context, DiscountService couponService, PaymentsService paymentsService) : BaseApiController
{
    [HttpGet]
    ...
    {
        var basket = await RetrieveBasket();
        if (basket == null || string.IsNullOrEmpty(basket.ClientSecret)) return BadRequest("Unable to apply vouvher");
    ...
    public async Task<ActionResult> RemoveCouponFromBasket()
    {
        ...
        return BadRequest("Porblem updating basket");
    }
